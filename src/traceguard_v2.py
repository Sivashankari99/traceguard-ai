"""
TraceGuard AI — Version 2

The only change from V1 is the retrieval layer, based on what the Module 2
vector-search notebook (`03-vector-search.ipynb`) actually validated:

  - Lexical search: MinSearch Index — unchanged from V1.
  - Semantic search: SentenceTransformer (all-MiniLM-L6-v2), normalized
    embeddings, cosine similarity — unchanged from V1.
  - Hybrid retrieval: lexical and semantic are each ranked over the FULL
    per-type population, then combined with real Reciprocal Rank Fusion
    (RRF) — no manual max()-of-scores merging of two differently
    calibrated score spaces.
  - Candidate selection: Top-K is taken AFTER fusion, and only that fused
    Top-K per type is passed forward into traceability expansion.
  - Retrieval diagnostics: lexical / semantic / hybrid retrieved counts,
    traceability seed count, and final candidate count sent to the LLM.

Everything downstream is identical to V1 and untouched:
traceability graph construction, traceability expansion, LLM context
selection, the LLM prompt, the LLM call, grounding validation, baseline
determination, and impact report generation.

No alternate retrieval backends, no runtime backend switching, no smoke
tests, no fallback logic — those were engineering scaffolding from the
notebook's experiments, not part of the validated best model.
"""
...
class TraceGuard:
    """Hybrid lexical + semantic (RRF-fused) + traceability-aware impact analyzer."""
    ...
    DEFAULT_RRF_K = 60
    HYBRID_SOURCE = "Hybrid RRF (lexical + semantic)"
    ...
    def _lexical_full_ranking(self, query_text, artifact_type):
        ...
        docs = self.documents_by_type.get(artifact_type, [])
        if idx is None or not docs:
            return []
        results = idx.search(query=query_text, num_results=len(docs))
        output = []
    ...
    def _embed_query(self, query_text):
        """Encode the query once per analyze() call; callers pass the
        result into _semantic_full_ranking() instead of re-encoding per
        artifact type."""
        return self.embedding_model.encode(
            [query_text], normalize_embeddings=True, show_progress_bar=False
        )[0]

    def _semantic_full_ranking(self, artifact_type, query_embedding):
        """Full per-type semantic ranking via normalized MiniLM cosine similarity, unchanged from V1."""
    ...
    @staticmethod
    def _reciprocal_rank_fusion(*ranked_id_lists, rrf_k=60):
        """RRF over full rankings — fuses rank position, never raw scores
        from differently-calibrated spaces (TF-IDF vs. cosine)."""
        scores = defaultdict(float)
        for ranked in ranked_id_lists:
            seen = set()
            for rank, artifact_id in enumerate(ranked, start=1):
                artifact_id = str(artifact_id)
                if artifact_id in seen:
                    continue
                seen.add(artifact_id)
                scores[artifact_id] += 1.0 / (rrf_k + rank)
        return [aid for aid, _ in sorted(scores.items(), key=lambda x: x[1], reverse=True)]

    def _hybrid_retrieve(self, query_text, artifact_type, query_embedding):
        """Lexical + semantic full rankings, fused with RRF, Top-K taken after fusion.

        `query_embedding` is computed once in analyze() (via _embed_query)
        and passed in here, so the query is never re-encoded per type.

        Returns (candidate_rows, diagnostics) where candidate_rows carries
        the fused hybrid_rank plus the underlying lexical/semantic evidence
        for that artifact (for display/grounding), and diagnostics reports
        the size of each population involved.
        """
        lexical_results = self._lexical_full_ranking(query_text, artifact_type)
        semantic_df = self._semantic_full_ranking(artifact_type, query_embedding)

        lexical_ids = [str(r["ID"]) for r in lexical_results]
        semantic_ids = semantic_df["ID"].astype(str).tolist() if not semantic_df.empty else []

        fused_ids = self._reciprocal_rank_fusion(lexical_ids, semantic_ids, rrf_k=self.rrf_k)

        retain_k = self.per_type_candidates.get(artifact_type, self.default_per_type_candidates)
        top_ids = fused_ids[:retain_k]

        lexical_lookup = {str(r["ID"]): r for r in lexical_results}
        semantic_lookup = (
            semantic_df.set_index(semantic_df["ID"].astype(str)).to_dict(orient="index")
            if not semantic_df.empty else {}
        )

        rows = []
        for hybrid_rank, artifact_id in enumerate(top_ids, start=1):
            base = self.artifact_lookup.get(artifact_id)
            if base is None:
                continue
            row = dict(base)
            row["hybrid_rank"] = hybrid_rank
            lex = lexical_lookup.get(artifact_id)
            row["lexical_score"] = lex.get("lexical_score", np.nan) if lex else np.nan
            row["lexical_rank"] = lex.get("lexical_rank", np.nan) if lex else np.nan
            sem = semantic_lookup.get(artifact_id)
            row["semantic_similarity"] = sem.get("semantic_similarity", np.nan) if sem else np.nan
            row["semantic_rank"] = sem.get("semantic_rank", np.nan) if sem else np.nan
            rows.append(row)
    ...
    def _is_traceability_seed(self, row):
        ...
        rank_ok = (
            rank_limit is not None and pd.notna(row.get("hybrid_rank"))
            and int(row["hybrid_rank"]) <= int(rank_limit)
        )
        similarity_ok = (
    ...
    # ------------------------------------------------------------------
    # Candidate ranking: single fused reciprocal-rank signal (from RRF)
    # plus the traceability bonus — no manual merging of two differently
    # calibrated raw scores.
    # ------------------------------------------------------------------
    ...
    def _review_rank(self, row):
        hybrid_rr = self._reciprocal_rank(row.get("hybrid_rank", np.nan))
        trace_signal = 1.0 if row.get("traceability_status") == "Linked" else 0.0
        return hybrid_rr + 0.10 * trace_signal
    ...
    # ------------------------------------------------------------------
    # Unchanged from V1: LLM context building, prompt, LLM call, grounding
    # validation, baseline determination.
    # ------------------------------------------------------------------
...
""".strip()
    ...
    # analyze() — Flow:
    #   Lexical Search ─┐
    #                    ├─► Hybrid RRF ─► Top-K Candidates
    #   Semantic Search ─┘
    #                            │
    #                            ▼
    #             Traceability Expansion (V1, unchanged)
    #                            │
    #                            ▼
    #                Candidate Ranking (fused rank + trace bonus)
    #                            │
    #                            ▼
    #             LLM Impact Assessment (V1, unchanged)
    #                            │
    #                            ▼
    #             Grounding Validation (V1, unchanged)
    #                            │
    #                            ▼
    #             Baseline Determination (V1, unchanged)
    # ------------------------------------------------------------------
    ...
    def analyze(self, query, call_llm=True):
        ...
        # Encode the query once for this analyze() call; every artifact
        # type reuses the same embedding instead of re-encoding it.
        ...
        # separate lexical/semantic evidence union.
        candidate_store = {}
        for artifact_type, rows in hybrid_results_by_type.items():
            for row in rows:
                artifact_id = str(row["ID"])
                base = {col: row.get(col, "") for col in self.DOCUMENT_COLUMNS}
                base.update({
                    "retrieval_sources": {self.HYBRID_SOURCE},
                    "traceability_paths": [],
                    "traceability_relationships": set(),
                    "semantic_similarity": row.get("semantic_similarity", np.nan),
                    "semantic_rank": row.get("semantic_rank", np.nan),
                    "lexical_score": row.get("lexical_score", np.nan),
                    "lexical_rank": row.get("lexical_rank", np.nan),
                    "hybrid_rank": row.get("hybrid_rank", np.nan),
                    "traceability_distance": np.nan,
                })
                candidate_store[artifact_id] = base
        ...
        for artifact_id, paths in discoveries.items():
            if artifact_id not in self.artifact_lookup:
                continue
            if artifact_id not in candidate_store:
                base_row = self.artifact_lookup[artifact_id]
                candidate_store[artifact_id] = {
                    **{col: base_row.get(col, "") for col in self.DOCUMENT_COLUMNS},
            ...
            item["retrieval_sources"].add("Traceability expansion")
            new_distance = min(p["distance"] for p in paths)
            item["traceability_distance"] = (
                new_distance if pd.isna(item["traceability_distance"])
                else min(item["traceability_distance"], new_distance)
            )
            for p in paths:
        ...
        for artifact_id, item in candidate_store.items():
            ...
            by_similarity = self.HYBRID_SOURCE in sources
        ...
        retrieval_diagnostics = {
            "per_type": pd.DataFrame(retrieval_diagnostics_rows),
            "traceability_seeds": len(seed_ids),
            "traceability_discovered": len(discoveries),
            "unlinked_similarity_candidates": int(candidates_df["unlinked_relevant"].sum()) if not candidates_df.empty else 0,
            "final_candidate_pool": len(candidates_df),
            "sent_to_llm": 0,  # filled in below once selection is known
        ...
        # --- LLM context selection: unchanged from V1 ---
        unlinked_budget = min(
        ...
        retrieval_diagnostics["sent_to_llm"] = len(selected)

        # --- Prompt, LLM call, grounding validation, baseline determination: unchanged from V1 ---
